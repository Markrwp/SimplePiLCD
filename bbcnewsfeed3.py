...
try:
    while True:
        ...
        for i in range(1): # Works best if you scroll only the first headline.
            w = 80 # Create an object called 'w' and use that to save character chunks of the RSS feed. Best with number of characters in headline.
            s = (bbc['entries'][i]['title']) # Define string of incoming news.
            s = f'{s: <{w}}' # Format string, with left justification.
            s = ''.join(s[i:i+80] for i in range(0, len(s), 80)) # Ensure string chunks are x characters long, joined with a space.
#            s = ((re.sub('([A-Z])', r' \1', s,))) # String 's', putting a space before every capital letter. Optional.
            ...
            if (len(s)>16): # If the length of the string is greater than 16 characters ...
                a,b = s[:40],s[40:] # Divide it into 2 parts, a & b, each of half the width of the feed.
